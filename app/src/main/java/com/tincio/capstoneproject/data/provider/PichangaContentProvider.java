...
public class PichangaContentProvider extends ContentProvider {
    ...
    private static final String TAG = PichangaContentProvider.class.getName();
    ...
    public static UriMatcher buildUriMatcher() {
        ...
        // Add URI matches
        uriMatcher.addURI(SoccerFieldContract.AUTHORITY, SoccerFieldContract.PATH_FIELD, RECIPES);
   //     uriMatcher.addURI(SoccerFieldContract.AUTHORITY, SoccerFieldContract.PATH_FIELD + "/#", RECIPE_WITH_ID);
        return uriMatcher;
    }

    // Member variable for a RECIPEDbHelper that's initialized in the onCreate() method
    private PichangaDB mDataBase;

    @Override
    public boolean onCreate() {
        Context context = getContext();
        mDataBase = new PichangaDB(context);
        return true;
    }
    ...
    public Uri insert(@NonNull Uri uri, ContentValues values) {
        ...
        switch (match) {
            case RECIPES:
                ...
                long id = mDataBase.getWritableDatabase().insert(SoccerFieldContract.SoccerEntry.TABLE_NAME, null, values);
                if (id > 0) {
                    returnUri = ContentUris.withAppendedId(SoccerFieldContract.SoccerEntry.CONTENT_URI, id);
                } else {
        ...
        switch (match) {
            ...
            case RECIPES:
               ...
               // mDataBase.getOpenHelper().getWritableDatabase().quer
                retCursor = mDataBase.getReadableDatabase().query(SoccerFieldContract.SoccerEntry.TABLE_NAME,
                        projection,
                        selection,
                        selectionArgs,
                        null,
    ...
    public int delete(@NonNull Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            ...
            case RECIPE_WITH_ID:
                ...
                RECIPEsDeleted = mDataBase.getWritableDatabase().delete(SoccerFieldContract.SoccerEntry.TABLE_NAME, "_id=?", new String[]{id});
                break;
            default:
                throw new UnsupportedOperationException("Unknown uri: " + uri);
        }
    ...
    public int update(@NonNull Uri uri, ContentValues values, String selection,
     ...
     //   final SQLiteDatabase db = mRECIPEDbHelper.getWritableDatabase();
        ...
        switch (match) {
            case RECIPES:
                RECIPEsUpdated = mDataBase.getWritableDatabase().update(SoccerFieldContract.SoccerEntry.TABLE_NAME, values, selection, selectionArgs);
                break;
            case RECIPE_WITH_ID:
                if (selection == null) selection = SoccerFieldContract.SoccerEntry._ID + "=?";
                else selection += " AND " + SoccerFieldContract.SoccerEntry._ID + "=?";
